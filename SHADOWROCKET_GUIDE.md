...
3. **重要：导入后必须验证配置**
   - 点击节点右侧的 `i` 图标
   - 核对UUID是否与服务器显示完全一致
   - 特别注意：`8` 和 `0`、`B` 和 `8` 等易混淆字符
4. 如果发现UUID不匹配，请手动修正或重新导入
...
```
服务器: ttjj11233.duckdns.org 或 服务器IP
端口: 443
UUID: 与服务器显示完全一致（特别注意8和0的区别）
加密: auto
传输: WebSocket + TLS
```

4. **UUID验证关键点**：
   - 完整的UUID应该是36位字符：`xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx`
   - 分段检查：8-4-4-4-12的格式
   - 易混淆字符：`8` vs `0`，`B` vs `8`，`l` vs `1`，`O` vs `0`
...
如果扫码后无法连接，可能是二维码识别错误：

| 错误现象 | 可能原因 | 解决方法 |
|---------|---------|---------|
| `proxy/vmess: Not Found` | UUID错误 | 手动修正UUID |
| 连接超时 | 地址/端口错误 | 检查服务器地址 |
| TLS握手失败 | TLS配置错误 | 确认TLS开启 |

**最佳实践**：建议优先使用配置文件导入或手动配置，避免扫码识别错误。
